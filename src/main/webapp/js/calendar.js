...
;(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    ...
    getInfo: function (obj) { // hover 时显示当天信息
      var _dateStr = $(obj).attr('data');
      var changeStr = _dateStr.substr(0, 4) + '-' + _dateStr.substr(4, 2) + '-' + _dateStr.substring(6);
      return changeStr;
    },
    ...
    showCalendar: function () { // 输入数据并显示
      ...
      this.$calendarYear_text.text(year);
      this.$calendarMonth_text.text(dateStr.substr(4, 2));
      // this.$calendarMonth_text.text(year + '/' + dateStr.substr(4, 2));
    ...
    renderDOM: function () { // 渲染DOM
      this.$calendar_year = $('<div class="calendar-year"></div>');
      ...
      var _yearStr = '<a href="#" class="year"></a>';
      var _monthStr = // '<a href="javascript:;" id="backToday">T</a>'+
                      '<div class="arrow">'+
      ...
      this.$calendar_year.html(_yearStr);
      this.$calendar_month.html(_monthStr);
      this.$calendar_week.html(_weekStr);
      this.$calendar_date.html(_dateStr);
      this.$calendar_today.html(_dayStr);

      this.$calendar.append(this.$calendar_year, this.$calendar_month, this.$calendar_week, this.$calendar_date, this.$calendar_today);
      this.$calendar.show();
    },
    ...
    inital: function () { // 初始化
      ...
      this.$calendarYear_text = this.$calendar_year.find('.year');
      this.$calendarMonth_text = this.$calendar_month.find('.month');
      // this.$backToday = $('#backToday');
      this.$arrow_prev = this.$calendar_month.find('.arrow-prev');
      this.$arrow_next = this.$calendar_month.find('.arrow-next');
