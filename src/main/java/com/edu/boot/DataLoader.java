...
public class DataLoader implements CommandLineRunner {
    ...
    @Autowired DerivedProductRepository derivedProductRepository;
    @Autowired OrderRepository orderRepository;
    @Autowired ImageCollectionRepository imageCollectionRepository;
    @Autowired ImageRepository imageRepository;
    @Autowired ImageServiceImpl imageServiceImpl;
    @Autowired CourseCategoryRepository courseCategoryRepository;
    @Autowired CourseProductRepository courseProductRepository;
    ...
    public void run(String... args) throws Exception {
        ...
        if (0 == courseCategoryRepository.count()) {
            CourseCategory courseCategory1 = new CourseCategory("绘本课", 24, new BigDecimal(2400), images.stream().map(x -> x).collect(Collectors.toCollection(HashSet::new)));
            courseCategories.add(courseCategoryRepository.save(courseCategory1));
//        		CourseCategory courseCategory2 = new CourseCategory("素描课", 24, new BigDecimal(2400), images.stream().map(x -> x).collect(Collectors.toCollection(HashSet::new)));
//        		courseCategories.add(courseCategoryRepository.save(courseCategory2));
        ...
        if (0 == productRepository.count()) {
            ...
            Product derivedProduct = new Product("T恤衍生品", productCategories.get(1), new BigDecimal(100), "衍生品", true, false);
            imageSet.clear();
            imageSet.add(images.get(1));
            derivedProduct.setProductImages(imageSet);
            products.add(productRepository.save(derivedProduct));
            Product imageCollectionProduct = new Product("作品集", productCategories.get(2), new BigDecimal(500), "作品集",false,true);
            imageSet.clear();
